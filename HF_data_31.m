...
zero_line = 126.4;
num_zero = 0;
shift_list = zeros(length(A_0_new),1);
index_list = zeros(length(A_0_new),1);
for j = 1:length(A_0_new)-1
    if (A_0_new(j)-zero_line)*(A_0_new(j+1)-zero_line)<=0
        num_zero = num_zero + 1;
        index_list(num_zero)=j;
        if A_0_new(j)<A_0_new(j+1)
            shift_list(num_zero)=1;       % 1代表峰
        elseif A_0_new(j)>A_0_new(j+1)
...
p3 = plot(0.0307098, 3.36643, 'o', 'MarkerSize', 8,'MarkerFaceColor', 'cyan', 'MarkerEdgeColor', 'black');
text(text_x_values(1), text_y_values(1)+0.0005, "time1 = " + text_x_values(1) + " s", 'FontSize', 12, 'FontName', 'Times New Roman');
p4 = plot(0.0528914, 3.35986, 'o', 'MarkerSize', 8,'MarkerFaceColor', 'green', 'MarkerEdgeColor', 'black');
text(0.0538914+0.0003, 3.35986-0.0003, "v min = 3.360 m/s", 'FontSize', 12, 'FontName', 'Times New Roman');
...
title('线速度随时间变化（HF信号，有均衡器）', 'FontSize', 25, 'FontName', 'Songti SC');